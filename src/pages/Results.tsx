...
// Derive display color from the text label (no hardcoded scores)
const getUrgencyColor = (label: string | undefined): string => {
  if (!label) return "text-primary";
  const u = label.toUpperCase();
  if (u.includes("CRITICAL")) return "text-destructive";
  if (u.includes("PRIORITY")) return "text-orange-400";
  return "text-primary"; // ROUTINE
};
...
const Results = () => {
  ...
  // Fetch real patient data from DB
  useEffect(() => {
      ...
      try {
        ...
        if (data) {
          setDynamicLogs([
            ...
            `Whole tumor detected: ${data.whole_tumor_volume} cm³`,
            `Enhancing core: ${data.enhancing_tumor_volume} cm³`,
            `Urgency assessment: ${data.urgency_score}`,
            "Analysis complete.",
          ]);
        }
      } catch (err) {
  ...
  const urgencyColor = getUrgencyColor(patientData?.urgency_score);
            ...
            <div className="bento-cell flex flex-col items-center justify-center overflow-hidden p-6" style={{ minHeight: 360 }}>
              <div className="relative flex h-full w-full items-center justify-center">
                ...
                <div className="relative flex flex-col items-center gap-2">
                  ...
                  <AnimatePresence>
                    {/* Show tumor overlay only when there's actual tumor volume */}
                    {showSegmentation && patientData && patientData.whole_tumor_volume > 0 && (
                      <motion.div
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="container mx-auto px-6 mt-16">
              <div className="mx-auto max-w-3xl">
                ...
                <div className="flex flex-col items-center gap-8">
                  {/* Sonar ring — numeric score + label */}
                  <motion.div className="relative flex h-48 w-48 items-center justify-center">
                    ...
                    <div className="relative z-10 flex h-32 w-32 flex-col items-center justify-center rounded-full border-2 border-primary/50 bg-primary/10 glow-border">
                      {patientData ? (
                        <>
                          <span className={`text-4xl font-bold ${urgencyColor}`}>
                            {patientData.urgency_numeric ?? "—"}
                          </span>
                          <span className={`text-xs font-medium uppercase tracking-widest ${urgencyColor} opacity-80`}>
                            {patientData.urgency_score?.includes("CRITICAL")
                              ? "CRITICAL"
                              : patientData.urgency_score ?? "—"}
                          </span>
                        </>
                      ) : (
                  ...
                  {/* Volume metrics — straight from DB, clearly labelled */}
                  <div className="grid w-full max-w-md grid-cols-3 gap-4">
                    {[
                      { label: "Whole Tumor",  value: patientData ? `${patientData.whole_tumor_volume} cm³`     : "—" },
                      { label: "Tumor Core",   value: patientData ? `${patientData.tumor_core_volume} cm³`      : "—" },
                      ...
                      <div key={m.label} className="bento-cell text-center p-4">
                        ...
                        <p className="mt-1 text-[10px] text-muted-foreground uppercase tracking-tight">{m.label}</p>
                      </div>
                    ))}
                  </div>

                  {/* Full urgency string as a badge below for clarity */}
                  {patientData?.urgency_score && (
                    <div className={`rounded-full border px-6 py-2 text-sm font-semibold uppercase tracking-widest ${urgencyColor} border-current/30`}>
                      {patientData.urgency_score}
                    </div>
                  )}

                  <div className="flex gap-4">
                    <button onClick={handleRescan} className="rounded-full border border-border px-8 py-3 font-semibold text-foreground hover:border-primary/50 hover:text-primary transition-all">
                      Rescan
                    </button>
                    <button onClick={handleScanAnother} className="rounded-full bg-primary px-8 py-3 font-semibold text-primary-foreground hover:bg-primary/90 transition-all">
                      Scan Another
                    </button>
                  </div>
                </div>
